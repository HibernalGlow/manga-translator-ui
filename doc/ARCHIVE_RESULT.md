...
5. **编码兼容性**：
   - 创建的 ZIP 文件使用 UTF-8 编码（Python 3.11+ 自动设置 UTF-8 标志位）
   - 支持日文、中文、韩文等多语言文件名
...
### UTF-8 编码
- **创建 ZIP**：自动使用 UTF-8 编码，设置 UTF-8 标志位（bit 11）
- **读取 ZIP**：自动检测 UTF-8 标志位，正确解码文件名
- **兼容性**：支持 Python 3.11+ 的默认 UTF-8 编码
...
### 编码检测
提供 `decode_zip_filename()` 函数处理非 UTF-8 编码的 ZIP 文件：
1. 检查 UTF-8 标志位
2. 尝试 Shift-JIS 解码（日文）
3. 尝试 GBK 解码（简体中文）
4. 尝试 Big5 解码（繁体中文）
5. 返回原始文件名（兜底）
